...
import ora from "ora";
import { component, name, folder, withinDir } from "./prompts";
import {
  getComponentName,
  getComponentFolder,
  getComponentFiles,
...
const scan = async (root = process.cwd()) => {
  ...
  const spinner = ora(
    `Scanning ${green(absoluteRoot)} for React component files...`
  ).start();
  ...
  if (!files.length) {
    ...
    console.log(
      `Make sure you are running ${cyan(
        "generact"
      )} inside a React-like project directory or using ${green(
        "root"
      )} option:\n`
    );
    console.log(
    ...
    console.log(
      "In this case, you can explicitly pass the component path to replicate:\n"
    );
    console.log(
      `    ${cyan("$ generact")} ${green(
        "relative/or/absolute/path/to/my/react/component.js"
      )}\n`
    );
